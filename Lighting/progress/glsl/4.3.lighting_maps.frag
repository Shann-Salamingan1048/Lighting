...
struct Light {
    ...
    vec3 position;  // Set via setVec3("light.position", x,y,z)
...
void main()
{
    ...
    // Specular (MODIFIED - inverted texture with crack preservation)
    vec3 viewDir = normalize(viewPos - FragPos);
    vec3 reflectDir = reflect(-lightDir, norm);
    float spec = pow(max(dot(viewDir, reflectDir), 0.0), material.shininess);
    
    // Invert the specular map but preserve subtle cracks (0.2 threshold)
    vec3 specMap = texture(material.specular, TexCoords).rgb;
    vec3 invertedSpecMap = 1.0 - smoothstep(0.0, 0.2, specMap); // smoothstep the one who inverted
    vec3 specular = light.specular * spec * invertedSpecMap;
